...
const SET_CALL_PREFERENCE = '@andyet/SET_CALL_PREFERENCE';
...
class SFU extends EventEmitter {
    ...
    createPeerConnection(session, options) {
        if (this.enabled && session.isInitiator) {
            ...
            return new ProxyPeerConnection(session.sid, room.id, room.address, peer.id, this, this.client);
        }
        else {
...
class ProxyPeerConnection extends EventEmitter {
    constructor(id, roomId, roomAddress, peerId, sfu, client) {
        super();
        this.id = id;
        this.roomId = roomId;
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                this.dispatch(joinRoomFailed(roomAddress, {
                    ...
                    roomFull: err.error && err.error.condition === 'service-unavailable',
...
class Hark extends EventEmitter {
    constructor(audioStream, opts = {}) {
        ...
        this.stoppedReceivingVolumeThreshold = Date.now() + 1000;
        if (!AudioContext) {
            return;
        }
        ...
        this.intervalTimer = setInterval(() => {
            ...
            if (stoppedReceivingVolume !== this.stoppedReceivingVolume) {
                if (stoppedReceivingVolume &&
                    stoppedReceivingVolume > this.stoppedReceivingVolumeThreshold) {
                    this.emit('stopped-receiving-volume', stoppedReceivingVolume);
                }
                else {
                    this.emit('started-receiving-volume');
                    this.stoppedReceivingVolumeThreshold = now + 1000;
                }
            }
 ...
 * @param replaces
 */
function addLocalAudio(track, stream, replaces, monitorVolume = true) {
    if (track.kind !== 'audio') {
    ...
    return (dispatch, getState) => {
        ...
        if (newMedia) {
            ...
            if (newMedia.source === 'remote' && newMedia.owner) {
                ...
                const allPeerMedia = getMediaForPeer(newState, newMedia.owner);
                let hasActiveMicrophone = false;
                let hasActiveCamera = false;
                let hasActiveScreenCapture = false;
                for (const media of allPeerMedia) {
                    if (!media.remoteDisabled) {
                        if (media.kind === 'audio') {
                            hasActiveMicrophone = true;
                        }
                        else if (media.screenCapture) {
                            hasActiveScreenCapture = true;
                        }
                        else {
...
function updateRoom(state, action) {
    ...
    if (action.type === JOIN_ROOM_FAILED) {
        ...
        return Object.assign(Object.assign({}, state), { [action.payload.roomAddress]: Object.assign(Object.assign({}, existingRoom), { banned: !!action.payload.banned, joined: false, joinedAt: undefined, password: '', passwordRequired: !!action.payload.passwordRequired, roomFull: !!action.payload.roomFull, roomNotStarted: !!action.payload.roomNotStarted, roomState }) });
    }
...
class Audio extends Component {
    shouldComponentUpdate(newProps) {
        return (!this.props.media ||
            !newProps.media ||
            this.props.media.localDisabled !== newProps.media.localDisabled ||
            this.props.media.remoteDisabled !== newProps.media.remoteDisabled ||
            this.props.media.track !== newProps.media.track ||
            this.props.media.id !== newProps.media.id ||
            this.props.volume !== newProps.volume ||
...
function mapStateToProps(state, props) {
    ...
    let peers = getPeersForRoom(state, props.room);
    if (props.includeSelf) {
        const user = getUserDataForRoom(state, props.room);
...
class PeerControls extends Component {
    render() {
        const renderProps = {
            ...
            hasActiveScreenCapture: this.props.hasActiveScreenCapture,
...
function mapStateToProps$8(state, props) {
    ...
    return {
        hasActiveCamera: peer.hasActiveCamera || false,
        hasActiveMicrophone: peer.hasActiveMicrophone || false,
        hasActiveScreenCapture: peer.hasActiveScreenCapture || false,
        ...
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                if (audio) {
                    ...
                    let volumeMonitoring = true;
                    if (this.props.volumeMonitoring !== undefined) {
                        volumeMonitoring = this.props.volumeMonitoring;
                    }
                    this.props.addLocalAudio(audio, stream, volumeMonitoring);
                    if (this.props.share !== false) {
...
function mapDispatchToProps$7(dispatch) {
    return {
        addLocalAudio: (track, stream, volumeMonitoring) => dispatch(addLocalAudio(track, stream, undefined, volumeMonitoring)),
    ...
    getMedia(additional = {}) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (audio) {
                ...
                this.props.addLocalAudio(audio, stream, this.props.replaceAudio, volumeMonitoring);
                if (this.props.share !== false) {
                    this.props.shareLocalMedia(audio.id);
                }
            }
...
function mapDispatchToProps$8(dispatch) {
    return {
        addLocalAudio: (track, stream, replace, volumeMonitoring) => dispatch(addLocalAudio(track, stream, replace, volumeMonitoring)),
...
function mapStateToProps$e(state, props) {
    ...
    const localMedia = !props.excludeMedia ? getLocalMedia(state) : undefined;
    const remoteMedia = !props.excludeMedia ? getRemoteMedia(state) : undefined;
    const peers = room && !props.excludePeers ? getPeersForRoom(state, room === null || room === void 0 ? void 0 : room.address) : undefined;
    return {
...
class Video extends Component {
    shouldComponentUpdate(newProps) {
        return (!this.props.media ||
            ...
            this.props.media.localDisabled !== newProps.media.localDisabled ||
            this.props.media.remoteDisabled !== newProps.media.remoteDisabled ||
            this.props.media.loaded !== newProps.media.loaded ||
            this.props.media.renderMirrored !== newProps.media.renderMirrored ||
            this.props.media.track !== newProps.media.track ||
            this.props.media.id !== newProps.media.id ||
            this.props.qualityProfile !== newProps.qualityProfile);
    }
    componentDidMount() {
        this.setup();
    }
...
function mapDispatchToProps$b(dispatch, props) {
    return {
        requestQualityProfile: (profile) => {
            if (props.media) {
                dispatch(requestQualityProfile(props.media.id, profile));
            }
        }
    };
}
